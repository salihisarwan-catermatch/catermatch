...
export default function OwnerPublic(){
  ...
  const websiteClean = useMemo(() => {
    if (!owner?.website) return null;
    return owner.website.startsWith('http') ? owner.website : `https://${owner.website}`;
  }, [owner]);
  ...
  if (owner.role !== 'owner') {
    return <div style={{padding:20}}>Dit is geen owner-profiel.</div>;
  }
